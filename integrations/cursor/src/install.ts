// ============================================================================
// DevCortex Cursor installer.
//
// `installCursor(targetRoot, { force })` writes / merges the DevCortex Cursor
// configuration into a target repository:
//   - `.cursor/rules/devcortex.mdc` — a Cursor project rule (MDC format) that
//                                      embeds the DevCortex discipline. Fully
//                                      DevCortex-managed, so it is overwritten
//                                      wholesale when regenerated.
//   - `.cursor/mcp.json`            — registers the `devcortex-mcp` stdio server
//                                      (merged, never clobbering foreign servers
//                                      or unrelated keys).
//
...
// strict: a malformed existing `.cursor/mcp.json` throws a DevCortexError rather
...
interface ManagedFile {
  ...
  exists: boolean;
  current: string | null;
  /** POSIX mode bits to write with. */
  mode: number;
}

// --- Boundary validation schemas (zod) --------------------------------------
//
// We validate only the SHAPE we touch: `mcpServers` must, if present, be an
// object. `looseObject` preserves every other key so merging never drops
...
async function buildRuleManagedFile(root: string): Promise<ManagedFile> {
  const absPath = join(root, ...CURSOR_RULE_PATH.split('/'));
...
async function buildMcpManagedFile(root: string): Promise<ManagedFile> {
  const absPath = join(root, ...CURSOR_MCP_PATH.split('/'));
  const relPath = relative(root, absPath) || absPath;
  const current = await readFileIfExists(absPath);
...
function changeAction(file: ManagedFile): InstallChangeAction {
  ...
  // The `.mdc` rule is fully DevCortex-managed → overwritten wholesale.
  // The MCP registry is merged so foreign servers survive.
  return file.role === 'rule' ? 'overwrite' : 'merge';
}

function planReason(file: ManagedFile): string {
  if (!file.exists) return `${file.relPath} does not exist yet and would be created`;
  const verb = file.role === 'rule' ? 'overwritten' : 'merged';
...
/**
 * Installs (or plans the installation of) the DevCortex Cursor project rule and
 * MCP registration into `targetRoot`.
 *
 * @throws {ConfigError} when an existing `.cursor/mcp.json` is not valid JSON or
 *   has an incompatible shape (never silently overwritten).
 * @throws {DevCortexError} on unexpected filesystem failures.
 */
export async function installCursor(
  targetRoot: string,
  options: InstallCursorOptions = {},
): Promise<InstallResult> {
  if (typeof targetRoot !== 'string' || targetRoot.trim() === '') {
    throw new DevCortexError('INTERNAL', 'installCursor requires a non-empty target root path.');
  }
